...
class Engine:
    ...
    def __init__(self, config: dict) -> None:
        ...
        config_file_path = join_paths(self.root_output_dir, "config.yaml")
        self.logger.info(
    ...
    def start(self) -> None:
        ...
        for workflow in self.workflows:
            ...
            if self.is_extension and workflow_name not in self.workflows_to_run.keys():
                self.logger.info(f"Skipping workflow: {workflow_name} (extension)")
                continue
            ...
            if not self.is_extension or (
                workflow_name in self.workflows_to_run.keys()
                and self.workflows_to_run[workflow_name] == 0
            ):
            ...
            for node in workflow["nodes"]:
                node_name = node["name"]
                node_dir = join_paths(workflow_dir, f"{node_counter}_{node_name}")
                ...
                if (
                    self.is_extension
                    and node_counter <= self.workflows_to_run[workflow_name]
                ):
                    self.logger.info(
